...
function renderUI (target, gltfScene, state) {
  ...
  const eventListeners = {
    addItem(inData, e) {
      ...
      if (data.length < child.userData.xravatar_maxCount) {
        data.push({
          name: child.name
        });
...
function formItem(category, eventListeners, state) {
  ...
  if (category.userData.xravatar_maxCount === 1) {
    const out = [];
    const hasData = !!state[category.name];
    const data = state[category.name] || {};
      ...
      <ul>
        ${category
          ...
          <li onclick="${eventListeners.addItem({
            child, data, category, state
          })}" class="${'oui-bubble-item button ' + (data.length > category.userData.xravatar_maxCount ? 'disabled' : '')}" tabindex="0" role="button" style="cursor: pointer;">
            <span class="plus">+</span>
            ${child.name}
          </li>`
          )}
      </ul>
      <h3>Children, ${data.length}/${category.userData.xravatar_maxCount} used.</h3>
      <ul>
        ${data.map(settingsEl)}
      </ul>
    `;
