...
import { hslVec3ToHex, hexToHslVec3 } from './colors.js';
...
function renderUI (target, uiData, uniforms, eventListeners) {
  ...
  function render(state) {
    ...
    const paletteForm = wire(uniforms.palette, 'html:panel')`
      <form id="Palette" role="tabpanel">
        ${uniforms.palette.map((p, i) => wire(p, 'html:colorpicker')`
          <p><label><input oninput=${function () {
            hexToHslVec3(this.value, p);
            eventListeners.onPaletteInput();
          }} type="color" name="colors" value="${hslVec3ToHex(p)}"> Colour ${i}</label></p>
        `)}
      </form>
    `;
      ...
      ${
        wire(data, 'html:dataitems')`<ul>
          <section data-category=${category}>
            ${data.children.map(item => settingsEl(
