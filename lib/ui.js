...
function settingsEl(itemUiData, eventListeners, item) {
  ...
  const {
    ...
    xravatar_mirror,
    xravatar_flip,
    xravatar_positionRadius,
    xravatar_positionPhi,
    xravatar_positionTheta
  } = item.settings;
  ...
  if (xravatar_canMirror) {
    ...
    out.push(wire(item, 'html:xravatar_canMirror')`
      <label><input type="checkbox" name="xravatar_mirror" checked=${xravatar_mirror}>Mirror Element</label>
  ...
  out.push(wire(item, 'html:xravatar_scale')`
    <label><input type="range" name="xravatar_scale" min=${xravatar_minScale} step=any max=${xravatar_maxScale} value=${xravatar_scale}>Scale Element</label>
  `);

  if (itemUiData.morphTargets) {
    itemUiData.morphTargets.forEach(key => {
      out.push(wire(item, 'html:xravatar_morph_' + key)`
        <label><input type="range" name="${'xravatar_morph_' + key}" min=0 step=any max=1 value=${item.settings['xravatar_morph_' + key] || 0}>${key}</label>
      `);
    });
  }

  if (xravatar_canMove) {
    out.push(wire(item, 'html:xravatar_position')`
      <label><input type="range" name="xravatar_positionRadius" min=${xravatar_minR} step=any max=${xravatar_maxR} value=${xravatar_positionRadius}>Distance</label>
      <label><input type="range" name="xravatar_positionPhi" min=${-Math.PI} step=any max=${Math.PI} value=${xravatar_positionPhi}>φ</label>
      <label><input type="range" name="xravatar_positionTheta" min=0 step=any max=${Math.PI} value=${xravatar_positionTheta}>θ</label>
    `);
  }
