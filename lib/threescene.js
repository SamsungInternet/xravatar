...
function renderGL(canvas, gltfScene) {
    ...
    function updateState(state, localstate) {
        ...
        if (localstate.get('threeRotateX')) xravatarRoot.rotation.y = localstate.get('threeRotateX')/300;
        if (localstate.get('threeRotateY')) xravatarRoot.rotation.x = localstate.get('threeRotateY')/-300;
    }
    return updateState;
}
