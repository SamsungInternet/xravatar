...
export default /* glsl */`
    ...
    float paletteTexelSize = 1.0/16.0;
    ...
    ${[
        ...
        index = floor(texelColor.r * 16.0) / 16.0 + paletteTexelSize * 0.5; // Between 0 and 1
        bshift =  vec4(vec3(0.5 + 16.0 * mod(texelColor.r, 0.0625)),1.0); // Between 0.5 and 1.5
        indexColor = texture2D( xravatar_palette, vec2(index, bshift) );
        outColor += indexColor * bshift * ${mix};`
    ).join('\n\n')}
