...
class AvatarLoader {
  ...
  async loadAvatarFromBase64(base64) {
    ...
    const array = [...string].map(c => charCodeAt(c) - 128);
    const json = await new Promise(resolve => LZMA.decompress(array, 6, resolve));
